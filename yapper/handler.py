...
def load_config(args: argparse.Namespace) -> dict:
    """Load a configuration from pyproject.toml."""
    ...
    if file_name is None:
        raise ValueError(
            'Yapper requires either a "--config" command-line parameter with a valid relative filepath '
...
def process_config(yap_config: dict) -> dict:
    ...
    if not isinstance(yap_config["module_map"], dict) or not len(yap_config["module_map"].keys()):
        raise TypeError(
            'The "module_map" key should be a "dict" type '

...
def test_process_config():
    ...
    with pytest.raises(KeyError):
        handler.process_config({"boo": "baa", 0: {"some.module": {"py": "boo.py", "astro": "baa.astro"}}})
    with pytest.raises(TypeError):
        handler.process_config({"boo": "baa", "module_map": {"some.module": "not a dict"}})
    # should raise if py or astro keys are missing
    with pytest.raises(KeyError):
        handler.process_config(
            {
                "boo": "baa",
                "module_map": {"some.module": {"-": "boo.py", "astro": "baa.astro"}},
            }
        )
    with pytest.raises(KeyError):
        handler.process_config(
            {
                "boo": "baa",
                "module_map": {"some.module": {"py": "boo.py", "-": "baa.astro"}},
            }
        )
    ...
    with pytest.raises(ValueError):
        handler.process_config(
            {
                "boo": "baa",
                "module_map": {"some.module": {"py": "boo", "astro": "baa.astro"}},
            }
        )
    with pytest.raises(ValueError):
        handler.process_config(
            {
                "boo": "baa",
                "module_map": {"some.module": {"py": "boo.py", "astro": "baa"}},
            }
        )
    ...
    with pytest.raises(KeyError):
        handler.process_config(
            {
                "boo": "baa",
                "module_map": {"some.module": {"py": "boo.py", "astro": "baa.astro"}},
            }
        )
...
def test_parse():
    ...
    astro = handler.parser.parse(
        module_name="tests.mock_file",
    ...
    yap_config = handler.load_config(args)
    merged_config = handler.process_config(yap_config)
    astro = handler.parser.parse(module_name="tests.mock_file", ast_module=ast_module, yap_config=merged_config)
